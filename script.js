      ...
      sto.updated = setTimeout(function () {
        ...
        if (typeof complete === "object") {
          ...
          if (!complete) {
            ...
            let old = sto.mask.querySelector(
              "[data-batch=i" + (sto.batch - 1) + "]"
            );
            if (old != null) {
            ...
            sto.mask
              .querySelector("[data-batch=i" + sto.batch + "]")
              .classList.add("taint");
            // remove old mat
        ...
        document.querySelector("section").setAttribute("data-load", complete);
        resolve(complete);
      }, 10);
      return complete;
    });
  ...
  utils.createFileFromUrl(
    ...
    function () {
      utils.createFileFromUrl(
        ...
        function () {
          sto.orient(
            "//images-na.ssl-images-amazon.com/images/I/61Jb9zMgMgL._SL1001_.jpg"
          );
        }
      );
    }
...
let cast = {
  ...
  layer: function (idx, name, mat) {
    ...
    let prev =
      sto.mask.querySelector("[data-batch=i" + (sto.batch - 1) + "]") ||
      document;
    ...
    let recolor =
      prev.querySelector("#" + name) ||
      document.querySelector("#default input[data-id=" + name + "]") ||
      document.querySelector(
        "#default input[data-id=" + name.replace(/(^f\d+_)|(_\d+)/g, "") + "]"
      );
  ...
  segment: async function (face) {
    ...
    let C = new cv.Point(face.x + face.width / 2, face.y + face.height / 2);
    ...
    cv.ellipse(
      mask,
      new cv.Point(C.x - face.width / 2, C.y + D),
      new cv.Size(face.width * 0.66, face.height * 1.0),
      180,
      90,
      -90,
      sto.GC.BGD,
    ...
    cv.rectangle(
      mask,
      new cv.Point(0, 0),
      new cv.Point(mask.cols, mask.rows),
      sto.GC.BGD,
      D / 2,
      4,
      0
    ...
    cv.line(
      mask,
      new cv.Point(C.x - face.width * 0.33, C.y),
      new cv.Point(C.x + face.width * 0.33, C.y),
      sto.GC.FGD,
      D,
      4,
    ...
    cv.circle(
      mask,
      new cv.Point(C.x, C.y - face.height / 2),
      D * 2,
      sto.GC.FGD,
      -1,
      4,
    ...
    cv.ellipse(
      mask,
      C,
      new cv.Size(face.width * 0.33, face.height * 0.66),
      0,
      -360,
      0,
      sto.GC.FGD,
  ...
  palette: async function (rect, lim) {
    ...
    if (diff.fit === false || (diff.fit && name === "hair")) {
      ...
      let min =
        name == "skin" ? sto.swatch[rect.idx + "skin"].skintone * 64 : 0;
      let off = {
        lo: cluster[0].hsv.v >= 32 + min ? cluster[0].rgb : cluster[1].rgb,
        hi: cluster[cluster.length - 2].rgb
      };

      let hi = new cv.Mat(tmp.rows, tmp.cols, tmp.type(), [
        off.hi.r + pad,
        off.hi.g + pad,
        off.hi.b + pad,
        255
      ...
      let lo = new cv.Mat(tmp.rows, tmp.cols, tmp.type(), [
        off.lo.r - pad,
        off.lo.g - pad,
        off.lo.b - pad,
        0
  ...
  diff: function (rect, face) {
    ...
    for (let y = mat.rows; y > 0; y -= int) {
      for (let x = mat.cols; x > 0; x -= int) {
        ...
        let dx = Math.round(
          255 - (Math.abs(x - mat.cols / 2) / mat.cols) * 255
        );
        ...
        hue =
          hue.max - hue.min > 90 ? hue.max - 180 - hue.min : hue.max - hue.min;
        hue = Math.abs(hue);
        ...
        if (!bandpass && (hue > 8 || prc > 0.25) && (sat > 32 || val > 32)) {
    ...
    for (let i = 0; i < contours.size(); ++i) {
      ...
      if (
        ...
        let off = {
          x: +Math.abs(
            ((rect.x + rect.width / 2 - cx) / rect.width) * 100
          ).toFixed(2),
          y: +Math.abs(
            ((rect.y + rect.height / 2 - cy) / rect.height) * 100
          ).toFixed(2)
        };
    ...
    if (cast.length) {
      cast.sort(function (a, b) {
        ...
        return (
          b.area.toFixed(2) - a.area.toFixed(2) ||
        ...
        if (bg <= 128) {
          if (inrange >= 128) {
            ...
            if (name === "hair") {
              ...
              if (sto.swatch[rect.idx + "lips"].mat.ucharAt(i, j) >= 128) {
                //todo: maybe all?
          ...
          if (name == "lips") {
            ...
            if (bg == 0 && skin >= 128) {
              // narrow head area
              cv.circle(mask, C, int, sto.GC.PR_FGD, -1, 4, 0);
              if (sat <= 64 - 32 * skintone) {
                // broad flesh (highlights * skintone)
  ...
  composite: function (complete) {
    ...
    if (complete !== 1) {
      ...
      let depth = new cv.Mat(size.width, size.height, cv.CV_8UC4, [
        64,
        64,
        64,
        255
...
function chart(complete) {
  ...
  if (d3.select("#wheel svg").size() < 1) {
    ...
    g = wheel.append("g").attr("transform", "translate(125, 125)");
    ...
    const spokePoint = (n) =>
      getPointAtAngle(
        angleScale(n) + angleScale.bandwidth() / 2 - 360 / slices / 2,
        cfg.steps[0].r + (cfg.steps[1].r - cfg.steps[0].r) / 2
      );
    ...
    const spoke = g
      ...
      .attr("transform", (n) => {
        const { x, y } = spokePoint(n);
        return `translate(${x * 1.25}, ${y * 1.25})`;
      });

    spoke.append("circle").attr("r", angleScale.bandwidth());

    spoke.append("text").attr("dy", "0.5em").text(angleScale);

    // origin
    g.append("line").attr("x1", 0).attr("x2", 0).attr("y1", 0).attr("y2", -100);
    ...
    function scheme() {
      ...
      for (let n = 0; n < slices; n++) {
        ...
        for (let j = 0; j < div; j++) {
          ...
          hsl[n].push(d3.hsl(h, 1 - j / div, 0.5));
  ...
  for (let j = 0; j < data.length; j++) {
    center
      ...
      .attr("class", "point")
      .attr("transform", (n, i) => {
        const { x, y } = getPointAtAngle(n.x, n.y);
        return `translate(${x}, ${y})`;
      })
      .attr("r", (d) => areaScale(Math.sqrt((d.count / Math.PI) * 2)))
      .attr(
        "fill",
        (d) => "rgb(" + d.rgb.r + "," + d.rgb.g + "," + d.rgb.b + ")"
      )
      .attr("id", (d) => d.id);
  }

  g.append("use").attr("id", "SVGorder").attr("xlink:href", "#");
}
...
function Pixi(complete) {
  ...
  ploader.add(
    "image" + sto.batch,
    document.getElementById("chroma").toDataURL("image/jpeg", 1.0)
  );
  ploader.add(
    "depth" + sto.batch,
    document.getElementById("depth").toDataURL("image/jpeg", 0.5)
  );
