...
let cast = {
	...
	layer: function (idx, name, mat) {
		...
		let prev =
			sto.mask.querySelector("[data-batch=i" + (sto.batch - 1) + "]") || document;
	...
	diff: function (rect, face) {
		...
		for (let i = 0; i < contours.size(); ++i) {
			...
			if (
				...
				let cy = M.m01 / M.m00;
				let off = {
					x: +Math.abs(((rect.x + rect.width / 2 - cx) / rect.width) * 100).toFixed(
						2
					),
					y: +Math.abs(
	...
	pyramid: function (rect) {
		...
		let skin_mat =
			name == "lips" || name == "hair" ? sto.swatch[rect.idx + "skin"].mat : false;
